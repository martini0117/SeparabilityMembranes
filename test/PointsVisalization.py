...
output_path = '/Users/shomakitamiya/Documents/python/snake3D/data/RemovedW10DataPara/'
...
for case in cases:
    ...
    # rect_size[0] = int(20 + 10*math.pow(np.sum(removed_seg)/500000,1/3))
    ...
    # cs = int(6 + 4*math.pow(np.sum(removed_seg)/500000,1/3))
    cs = 10

    surf = tb.cul_contour_seg(img,seg,surf,rect_size,div=div,N_limit=10,c=0.95,ctrlpts_size=cs,dif_limit=-0.01,dif_abs=False,w=0.95,ctrlpts_increasing=True,increasing_limit=0.005,weight=1.0,debug=True)
